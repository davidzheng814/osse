...
def collate(batch):
    x, y = zip(*batch)
    x = zip(*x)
    x = [torch.stack(tensors, 0) for tensors in x]
...
class PhysicsDataset(torch.utils.data.Dataset):
    def __init__(self, train=True):
        ...
        self.data_cache = [None for _ in range(len(self.files))]
...
kwargs = {'num_workers': 8, 'pin_memory': True} if args.cuda else {'num_workers': 4}
...
def test_epoch(epoch):
    ...
    print 'Test Loss (L1): {:.3f}'.format(tot_loss / num_samples)
