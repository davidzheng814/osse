    ...
    with tf.Session() as sess:
        ...
        for epoch in range(start_epoch, args.epochs+1):
            ...
            if not args.skip_train and not args.baseline:
                train_model.run(epoch, train_ind, sess, train_writer, test_model.epochs_wo_dec)
            train_ind = epoch * train_model.batches_per_epoch # Set train_ind for test summaries.
            test_model.run(epoch, train_ind, sess, test_writer, save_encs=True)
            #TODO None of these currently log to tensorboard
            #TODO Fix logging to look nicer, especially obj9 R^2
            obj3_model.run(epoch, train_ind, sess, test_writer, save_encs=True)
            obj9_model.run(epoch, train_ind, sess, test_writer, save_encs=True)
            mass32_model.run(epoch, train_ind, sess, test_writer, save_encs=True)
            long_model.run(epoch, train_ind, sess, test_writer, save_ro=True)

            epoch_time = time.time() - start_time
            log(args.log_dir, 'Time: {:.2f}s Epochs w/o Dec: {}\n'.format(epoch_time, test_model.epochs_wo_dec))

            if args.baseline or args.skip_train: # baseline only runs for one epoch
                break
